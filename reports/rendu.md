...
**Résultat :** MAE CV ≈ **13768** ; Kaggle ≈ **12882** (**meilleur score**)  
**Analyse :** ElasticNet peut “éteindre” un modèle redondant (coeff ≈ 0), ce qui confirme que certains modèles n’ajoutaient plus de signal unique.

---

### RUN 013 — Linear Stack (Ridge + Lasso) ajouté aux trees
**Idée :** jusqu’ici, le stack est composé de modèles à base d’arbres. Ajouter un modèle linéaire (Ridge/Lasso) apporte une **diversité architecturale** : les modèles linéaires et les arbres font des erreurs différentes, ce qui réduit la variance du méta-modèle.
- Ajout **Ridge (α=10)** et **Lasso (α=0.0005)** comme base models
- Pipeline dédié pour les modèles linéaires : `SimpleImputer → StandardScaler → OneHotEncoder`
- Correction bug skewness : liste des colonnes skewed calculée sur train puis appliquée au test
- Stack total : XGB + CatBoost + ExtraTrees + LGB + Ridge + Lasso → méta ElasticNet

**Résultat :** MAE CV ≈ **13264**, MAPE ≈ **7.63%** ; Kaggle ≈ **12608** (**meilleur score**)  
**Analyse :** l’ajout de modèles linéaires apporte un signal orthogonal aux trees. La diversité architecturale confirme son intérêt, et le meta-model ElasticNet choisit les poids optimaux sur 6 modèles.
...
5) **Diversité des modèles (trees + linéaires)** : amélioration nette sur la fin avec RUN 013
...
La meilleure performance (**12608** sur Kaggle public) est arrivée non pas grâce à un seul “truc magique”, mais grâce à la **combinaison des meilleures décisions** : données propres + stacking OOF + encodage + régularisation, et surtout l’ajout d’une **diversité architecturale** (modèles linéaires en plus des arbres) sur RUN 013.
...
| 012 | Tuning + ElasticNet meta | 13768 | 7.88% | 12882 | Meilleur final (avant RUN 013) |
| 013 | Linear Stack (Ridge+Lasso) | 13264 | 7.63% | **12608** | Meilleur final |