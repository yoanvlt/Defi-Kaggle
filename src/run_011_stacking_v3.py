...
SUBMISSION_PATH = "submissions/submission_011_stacking_v3.csv"
RESULTS_PATH = "results_run011.txt"
RANDOM_STATE = 42
N_SPLITS = 5

# Features to drop (quasi-constant or noisy)
...
def fix_skewness(df, threshold=SKEW_THRESHOLD):
    """
    Applique log1p aux features numériques avec une forte asymétrie (skewness > threshold).
    Cela normalise les distributions et aide les modèles linéaires et le meta-model.
    """
...
def target_encode_oof(train_df, test_df, col, target, kf):
    """
    Target encoding OOF: encode une variable catégorielle par la moyenne de la cible,
    calculée uniquement sur les folds d'entraînement (pas de leakage).
    
    Pour le test set: utilise la moyenne globale du train.
    """
    new_col = f"{col}_te"
    ...
    for train_idx, val_idx in kf.split(train_df, target):
        # Calculer la moyenne cible par catégorie sur le train fold uniquement
        means = target.iloc[train_idx].groupby(train_df[col].iloc[train_idx]).mean()
        # Appliquer aux données val fold
        train_df.loc[train_df.index[val_idx], new_col] = train_df[col].iloc[val_idx].map(means)
    
    # Remplir les NaN (catégories non vues) par la moyenne globale
    train_df[new_col] = train_df[new_col].fillna(global_mean)
    
    # Pour le test: utiliser la moyenne globale par catégorie
...
def get_oof_preds(model_name, model, X, y_log, X_test, kf, cat_features=None):
    """
    Génère les prédictions OOF pour le train et la moyenne des prédictions pour le test.
    """
    print(f"\n{'='*50}")
...
def main():
    print(f"Run 011: Stacking v3 (TE + Skewness + Feature Drop) started at {datetime.now()}")
    ...
    # 4. Global Cleaning + Ordinal Encoding + Feature Engineering
    def preprocess_global(df):
        df_clean = clean_data(df)
        df_ord = encode_ordinals(df_clean)
        df_feat = add_features(df_ord)
        return df_feat
    
    print("Applying Cleaning + Ordinals + Feature Engineering...")
    X_all = preprocess_global(X_train_raw)
    X_test_all = preprocess_global(X_test_raw)
    
    # 5. Target Encoding (OOF - no leakage)
    ...
    print(f"\nDropping noise features: {DROP_FEATURES}")
    ...
    # 7. Fix Skewness on numerical features
    print("\nFixing skewness...")
    ...
    # 10. Stacking (Meta Model — 4 base models)
    ...
    print(f"Meta Coefficients: {coefs}")
    ...
    print(f"\n*** FINAL RESULTS ***")
    print(f"Stacking v3 OOF MAE: {mae:.4f}")
    print(f"Stacking v3 OOF MAPE: {mape*100:.2f}%")
    print(f"(RUN 010 reference: MAE=13950.59, Kaggle=13074.70)")
    ...
    with open(RESULTS_PATH, "w") as f:
        f.write(f"Stacking v3 MAE: {mae:.4f}\n")
        f.write(f"Stacking v3 MAPE: {mape:.4f}\n")
        f.write(f"Meta Coefs: {coefs}\n")
        f.write(f"Improvements: OOF Target Encoding ({TARGET_ENCODE_COLS}), Skewness Fix, Feature Drop\n")
