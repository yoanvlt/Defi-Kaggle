...
SUBMISSION_PATH = "submissions/submission_013_linear_stack.csv"
RESULTS_PATH = "results_run013.txt"
RANDOM_STATE = 42
N_SPLITS = 5

# Features to drop (identiques run_012)
...
def fix_skewness(df, skewed_cols=None, threshold=SKEW_THRESHOLD):
    """
    Correction de la skewness.
    Si skewed_cols est fourni (liste), on applique log1p uniquement sur ces colonnes.
    Sinon, on détecte les colonnes skewed et on les retourne.
    """
    ...
    if skewed_cols is None:
        ...
        print(f"  Skewness correction: {len(skewed_cols)} features (threshold={threshold})")
...
def get_oof_preds(model_name, model, X, y_log, X_test, kf, cat_features=None):
    ...
    is_linear = "Linear" in model_name  # Ridge ou Lasso
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X, y_log)):
        ...
        if is_catboost:
            ...
            # Pipeline avec preprocesseur dédié incluant StandardScaler
            preprocessor = model.named_steps["preprocessor"]
            ...
            val_pred_log = linear_model.predict(X_val_processed)
            test_pred_log = linear_model.predict(X_test_processed)
        else:
...
def build_linear_preprocessor(numeric_cols, categorical_cols):
    """
    Preprocesseur dédié aux modèles linéaires :
    - StandardScaler sur le numérique (obligatoire pour Ridge/Lasso)
    - OneHotEncoder sur les catégorielles (reste identique)
    """
    return ColumnTransformer([
        ("num", Pipeline([
            ("imputer", SimpleImputer(strategy="median")),
            ("scaler", StandardScaler())          # Clé : normalisation pour Ridge/Lasso
        ]), numeric_cols),
        ("cat", Pipeline([
...
def build_tree_preprocessor(numeric_cols, categorical_cols):
    """
    Preprocesseur pour les modèles à base de trees (pas besoin de scaler).
    """
    return ColumnTransformer([
...
def main():
    print(f"Run 013: Linear Stack (Ridge + Lasso as base models) started at {datetime.now()}")
    ...
    # 7. Fix Skewness — IMPORTANT: calculer la liste sur le train, appliquer la même au test
    print("\nFixing skewness...")
    ...
    # --- C. ExtraTrees (identique RUN 012) ---
    model_et = Pipeline([
        ("preprocessor", tree_preprocessor),
    ...
    model_ridge = Pipeline([
        ...
        ("linear", Ridge(
            alpha=10.0,           # Régularisation L2 — cross-validated manuellement
            random_state=RANDOM_STATE
        ))
    ])
    ...
    model_lasso = Pipeline([
        ("preprocessor", linear_preprocessor),
        ("linear", Lasso(
            alpha=0.0005,         # Régularisation L1 douce (sélection de features)
            max_iter=10000,
            random_state=RANDOM_STATE
        ))
    ])

    # 10. Generate OOFs — tous les modèles
    ...
    # 11. Stacking méta — ElasticNet entraîné sur les 6 OOFs
    ...
    print(f"\n{'='*50}")
    print("Evaluating Meta-Models on 6-model stack...")
    ...
    # ElasticNet méta (L1 peut zeroing les modèles redondants)
    ...
    # 13. Sauvegarde résultats
    with open(RESULTS_PATH, "w") as f:
        f.write(f"RUN 013 — Linear Stack (Ridge + Lasso as base models)\n")
        ...
        f.write(f"Improvements: +Ridge & +Lasso base models (StandardScaler), skew fix aligned train/test\n")
